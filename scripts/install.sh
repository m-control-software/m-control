...
# Earlier installers copied an unused config template here; drop the leftover.
rm -f "$INSTALL_PATH/config.template.json"
...
echo ""
echo "Next steps:"
echo "  1. Run 'mctl doctor' - it lists every tool's missing required config"
echo "  2. Add those keys under \"tools\" in: $CONFIG_PATH (each tool's README lists its keys)"
echo ""
