...
Read `AGENTS.md` at the repo root first. It is the canonical guide for every
AI agent: layout, commands, contracts (manifest, Tool Protocol v1, config),
...
release, author device profiles) are Agent Skills in `.claude/skills/`, which
this assistant loads by itself; `AGENTS.md` → "Procedures (skills)" lists
them. Run `yarn verify` before calling work done — CI runs the same script.
